...
> [!TIP]
> <span data-ttu-id="e8611-104">可在 GitHub 上找到此文档中的代码作为可 [运行示例](https://github.com/dotnet/EntityFramework.Docs/tree/master/samples/core/Miscellaneous/Testing/ItemsWebApi/)。</span><span class="sxs-lookup"><span data-stu-id="e8611-104">The code in this document can be found on GitHub as a [runnable sample](https://github.com/dotnet/EntityFramework.Docs/tree/master/samples/core/Miscellaneous/Testing/ItemsWebApi/).</span></span>
...
<span data-ttu-id="e8611-109">该 [示例](https://github.com/dotnet/EntityFramework.Docs/tree/master/samples/core/Miscellaneous/Testing/ItemsWebApi/) 包含两个项目：</span><span class="sxs-lookup"><span data-stu-id="e8611-109">The [sample](https://github.com/dotnet/EntityFramework.Docs/tree/master/samples/core/Miscellaneous/Testing/ItemsWebApi/) contains two projects:</span></span>

- <span data-ttu-id="e8611-110">ItemsWebApi：通过单个控制器 [ASP.NET Core 支持](/aspnet/core/tutorials/first-web-api) 的非常简单的 Web API</span><span class="sxs-lookup"><span data-stu-id="e8611-110">ItemsWebApi: A very simple [Web API backed by ASP.NET Core](/aspnet/core/tutorials/first-web-api) with a single controller</span></span>
...
<span data-ttu-id="e8611-113">此 API 支持以下两种实体类型： Items 和 Tags 。</span><span class="sxs-lookup"><span data-stu-id="e8611-113">The model backing this API has two entity types: Items and Tags.</span></span>

- <span data-ttu-id="e8611-114">Items 具有区分大小写的名称和的集合 Tags 。</span><span class="sxs-lookup"><span data-stu-id="e8611-114">Items have a case-sensitive name and a collection of Tags.</span></span>
- <span data-ttu-id="e8611-115">每个 Tag 都有一个标签和一个计数，表示它应用到的次数 Item 。</span><span class="sxs-lookup"><span data-stu-id="e8611-115">Each Tag has a label and a count representing the number of times it has been applied to the Item.</span></span>
- <span data-ttu-id="e8611-116">每个 Item 仅应有一个 Tag 具有给定标签的。</span><span class="sxs-lookup"><span data-stu-id="e8611-116">Each Item should only have one Tag with a given label.</span></span>
...
- <span data-ttu-id="e8611-118">删除将 Item 删除所有关联的 Tags 。</span><span class="sxs-lookup"><span data-stu-id="e8611-118">Deleting an Item should delete all associated Tags.</span></span>

#### <a name="the-no-locitem-entity-type"></a><span data-ttu-id="e8611-119">Item实体类型</span><span class="sxs-lookup"><span data-stu-id="e8611-119">The Item entity type</span></span>

<span data-ttu-id="e8611-120">`Item`实体类型：</span><span class="sxs-lookup"><span data-stu-id="e8611-120">The `Item` entity type:</span></span>
...
- <span data-ttu-id="e8611-127">Tags 公开为 `IReadOnlyList<Tag>` 以防止任意修改。</span><span class="sxs-lookup"><span data-stu-id="e8611-127">Tags are exposed as a `IReadOnlyList<Tag>` to prevent arbitrary modification.</span></span>
  - <span data-ttu-id="e8611-128">EF `Tags` 通过匹配属性的名称将属性与 `_tags` 支持字段相关联。</span><span class="sxs-lookup"><span data-stu-id="e8611-128">EF associates the `Tags` property with the `_tags` backing field by matching their names.</span></span>
  - <span data-ttu-id="e8611-129">`AddTag`方法采用标签标签，并实现上面所述的业务规则。</span><span class="sxs-lookup"><span data-stu-id="e8611-129">The `AddTag` method takes a tag label and implements the business rule described above.</span></span>
...
- <span data-ttu-id="e8611-132">为 `Tags` 多对一关系配置导航属性</span><span class="sxs-lookup"><span data-stu-id="e8611-132">The `Tags` navigation property is configured for a many-to-one relationship</span></span>
  - <span data-ttu-id="e8611-133">不需要从到的导航属性 Tag Item ，因此不包含。</span><span class="sxs-lookup"><span data-stu-id="e8611-133">There is no need for a navigation property from Tag to Item, so it is not included.</span></span>
  - <span data-ttu-id="e8611-134">此外，不 Tag 会定义外键属性。</span><span class="sxs-lookup"><span data-stu-id="e8611-134">Also, Tag does not define a foreign key property.</span></span>
...
#### <a name="the-no-loctag-entity-type"></a><span data-ttu-id="e8611-136">Tag实体类型</span><span class="sxs-lookup"><span data-stu-id="e8611-136">The Tag entity type</span></span>

<span data-ttu-id="e8611-137">`Tag`实体类型：</span><span class="sxs-lookup"><span data-stu-id="e8611-137">The `Tag` entity type:</span></span>
...
<span data-ttu-id="e8611-139">与类似 Item ， Tag 隐藏其主键并使属性成为 `Label` 只读的。</span><span class="sxs-lookup"><span data-stu-id="e8611-139">Similarly to Item, Tag hides its primary key and makes the `Label` property read-only.</span></span>

### <a name="the-no-locitemscontroller"></a><span data-ttu-id="e8611-140">Items控制器</span><span class="sxs-lookup"><span data-stu-id="e8611-140">The ItemsController</span></span>
...
<span data-ttu-id="e8611-143">它具有获取所有 Items 或 Item 具有给定名称的方法：</span><span class="sxs-lookup"><span data-stu-id="e8611-143">It has methods to get all Items or an Item with a given name:</span></span>

[!code-csharp[Get](../../../samples/core/Miscellaneous/Testing/ItemsWebApi/ItemsWebApi/Controllers/ItemsController.cs?name=Get)]

<span data-ttu-id="e8611-144">它有一个用于添加新的方法 Item ：</span><span class="sxs-lookup"><span data-stu-id="e8611-144">It has a method to add a new Item:</span></span>
...
<span data-ttu-id="e8611-145">用标签标记的方法 Item ：</span><span class="sxs-lookup"><span data-stu-id="e8611-145">A method to tag an Item with a label:</span></span>
...
<span data-ttu-id="e8611-146">以及用于删除 Item 和全部关联的方法 Tags ：</span><span class="sxs-lookup"><span data-stu-id="e8611-146">And a method to delete an Item and all associated Tags:</span></span>
...
- <span data-ttu-id="e8611-158">`Can_remove_item_and_all_associated_tags` 与 EF 内存中数据库提供程序一起运行时</span><span class="sxs-lookup"><span data-stu-id="e8611-158">`Can_remove_item_and_all_associated_tags` when running with the EF in-memory database provider</span></span>
...
<span data-ttu-id="e8611-188">两个稍微复杂的测试涵盖了有关添加的业务逻辑 tags 。</span><span class="sxs-lookup"><span data-stu-id="e8611-188">Two slightly more involved tests cover the business logic around adding tags.</span></span>
...
<span data-ttu-id="e8611-194">应用程序的要求之一是 " Items 具有区分大小写的名称和集合 Tags "。</span><span class="sxs-lookup"><span data-stu-id="e8611-194">One of the requirements for our application is that "Items have a case-sensitive name and a collection of Tags."</span></span>
...
<span data-ttu-id="e8611-205">我们的应用程序的另一个要求是 "删除" Item 应删除所有关联的 Tags 。 "</span><span class="sxs-lookup"><span data-stu-id="e8611-205">Another of the requirements for our application is that "deleting an Item should delete all associated Tags."</span></span>
